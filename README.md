...
[📄 Build guide (PDF)](assets/Pi5-DECK-7inch-Build-Guide.pdf)
...
> [!TIP]
> microSD is the simplest start. Want speed later? Add an NVMe — see [Optional add-ons](#optional-add-ons).
...
| Item | Adds |
|---|---|
| M.2 HAT+ + 1TB NVMe (2242) | Fast storage |
...
**Fast storage (NVMe).** Add the official M.2 HAT+ and a 1TB **2242** NVMe (a 2242 drive is
required — the HAT+ won't fit a 2280). Migrate the OS off the microSD with
[`rpi-clone`](https://github.com/billw2/rpi-clone), or flash the image straight onto the SSD, then
set the bootloader to prefer NVMe:

```bash
sudo rpi-eeprom-config -e     # set BOOT_ORDER=0xf416 (NVMe first, SD fallback)
findmnt /                     # after reboot: should show /dev/nvme0n1p2
```

**Portability.** Run from a USB-C PD power bank for cordless use, or add a UPS HAT for an internal
battery.

**Enclosure.** Print or buy a case/stand to suit.
